...
namespace Kentico.Kontent.Boilerplate.Infrastructure
{
    public class NavigationProvider
    {
        ...
        public async Task<List<NavigationItem>> GetCachedNavigation(string rootNavigationCodename, int depth=1, int cacheTimespanInMinutes = 10)
        {
            string cacheKey = "CustomNavigationCache-" + Language;

            return await _cache.GetOrCreateAsync(cacheKey, async entry =>
            {
                var navigation = await GetNavigation(rootNavigationCodename, depth);

                entry.AbsoluteExpirationRelativeToNow = TimeSpan.FromMinutes(cacheTimespanInMinutes);
